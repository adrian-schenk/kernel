...
void pci_init()
{
  ...
  pci_devices = pt_alloc_page_phys(1);
...
static void pci_print_bar(uint32_t bar, int index)
{
  ...
  pci_devices[pci_device_count].bars[index] = (pci_bar_t){
      .base = base,
        ...
        if (class_code == 0x01 && subclass == 0x06)
        {
          ...
          for (int i = 0; i < 6; i++)
          {
            ...
            {              // memory-mapped BAR
              ...
              for (int j = 0; j < 64; j++)
              {
                pt_map_page(&kernel_pml4, (uintptr_t)bar + j * PAGE_SIZE, (uintptr_t)bar + j * PAGE_SIZE, PAGE_PRESENT | PAGE_WRITABLE);
              }
            }
          }
        }
